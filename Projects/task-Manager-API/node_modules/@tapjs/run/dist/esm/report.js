...
export const report = async (args, config) => {
    ...
    const htmlReport = reporter.includes('html') || reporter.includes('html-spa') ?
...
const isEmptyCoverage = (summary) => Math.max(...thresholds.map(th => Number(summary[th].pct) || 0)) === 0;
const isFullCoverage = (summary) => Math.min(...thresholds.map(th => Number(summary[th].pct) || 0)) === 100;
