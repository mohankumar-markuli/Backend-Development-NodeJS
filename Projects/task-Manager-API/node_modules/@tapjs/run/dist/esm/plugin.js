...
const detectGlobalInstall = async (args, project) => {
    ...
    const activeRunner = String(resolveImportSync('@tapjs/run', import.meta.url));
    const projectTap = tryResolveImport('tap', resolve(project, 'x'));
    const projectRunner = String(projectTap ?
...
This will likely fail. Try:
    ...
    ${chalk.green(`npm exec tap plugin ${args.map(a => JSON.stringify(a)).join(' ')}`)}
`);
    }
    /* c8 ignore stop */
};
export const plugin = async (args, config, quiet = false) => {
...
const add = async (args, config, quiet = false) => {
    ...
    try {
        ...
        if (!success) {
            ...
            if (installed && needCleanup.size) {
                ...
                await uninstall([...needCleanup], config).catch(() => error(chalk.red('uninstall failed')));
            }
        }
    }
};
