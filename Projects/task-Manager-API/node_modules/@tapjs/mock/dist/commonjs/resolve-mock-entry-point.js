"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.resolveMockEntryPoint = void 0;
const resolve_import_sync_1 = require("resolve-import/resolve-import-sync");
const resolveMockEntryPoint = async (url, module, serviceKey, key, caller) => {
    let mockedModuleURL;
    if (module.startsWith('./') || module.startsWith('../')) {
        mockedModuleURL = new URL(module, url);
    }
    else {
        const res = ((0, resolve_import_sync_1.resolveImportSync)(module, url));
        // caught at the exposed API, defense in depth only
        // but the experience if it throws here is unhelpful.
        /* c8 ignore start */
        if (typeof res === 'string') {
            const er = new TypeError('node builtins cannot be mock imported');
            Error.captureStackTrace(er, caller);
            throw er;
        }
        /* c8 ignore stop */
        mockedModuleURL = res;
    }
    mockedModuleURL.searchParams.set('tapmock', `${serviceKey}.${key}`);
    return String(mockedModuleURL);
};
exports.resolveMockEntryPoint = resolveMockEntryPoint;
//# sourceMappingURL=resolve-mock-entry-point.js.map