...
const resolveConfig = (target, from = process.cwd()) => {
    if (isAbsolute(target))
        return target;
    let local = resolve(from, target);
    if (!isFile(local) && isFile(local + '.json'))
        local += '.json';
    if (isFile(local) ||
        target.startsWith('.\\') ||
        target.startsWith('./')) {
...
const parseTsConfig = (configPath, seen = new Set()) => {
    try {
        const data = JSON.parse(readFileSync(resolveConfig(configPath), 'utf8'));
